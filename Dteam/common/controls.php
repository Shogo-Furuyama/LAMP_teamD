...
END_BLOCK;
	...
	/*!
		  ...
		  */
	//--------------------------------------------------------------------------------------
	public function get_website($pageparam, $tgt_page,$keyword, $first_ex = '', $last_ex = '', $att = '')
	{
		$str = "";
		$pagemax = (int) ($this->m_allcount / $this->m_limit);
		if ($this->m_allcount % $this->m_limit) {
		...
		for ($i = 1; $i <= $pagemax; $i++) {
			if ($i == $tgt_page) {
				...
				$temp = <<<END_BLOCK
            <li class="page-item"><a class="page-link" href="{$this->m_base_uri}{$pageparam}={$i}&w={$keyword}" {$att}>{$i}</a></li>
            END_BLOCK;
				$str .= $temp;
			}
		}
